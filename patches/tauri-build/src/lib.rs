...
fn copy_file(from: impl AsRef<Path>, to: impl AsRef<Path>) -> Result<()> {
  ...
  if to.exists() {
    remove_or_replace_dest(to)?;
  }
  fs::copy(from, to)?;
  Ok(())
}

/// Remove an existing build output (sidecar, bundled resource, etc.) before copying the new file.
/// On Windows, delete may fail with `PermissionDenied` / sharing errors if the file is still running or read-only.
fn remove_or_replace_dest(dest: &Path) -> Result<()> {
  #[cfg(windows)]
  ...
  let stale = dest.with_extension("old");
  let _ = fs::remove_file(&stale);
  if fs::rename(dest, &stale).is_ok() {
    let _ = fs::remove_file(&stale);
    return Ok(());
  }
  Err(anyhow::anyhow!(
    "Could not remove or rename {:?}. \
Stop the running Clash Verge / `cargo run` / `pnpm run dev` if this executable is loaded, then rebuild.",
    dest
  ))
}
